...
import { createCockpitPickerCommandMessage } from "@/components/terminals/contents/messages/message";
      ...
      .then(async () => {
        await modifyLastInputCommandSplit(async ({ command }) => {
          if (inputValue.value.endsWith(Blank) || !inputValue.value) {
            ...
            return commands.value = _commands;
          }
          if (!command) return;
          ...
          commands.value = [await createCockpitPickerCommandMessage(command)];
        });
      })
      .then(focusInput);
  }
